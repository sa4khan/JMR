...
target += status .* (log(kappa) + kappa * log(rho) + (kappa-1) * log(st) + pred1)
   - (to_matrix(exp((kappa-1) * log(nodes1) + phi * (nodesf * alpha 
   + row_sums(nodesr .* row_expan0(b,qpoints)))), m, qpoints, 0) * weights) 
