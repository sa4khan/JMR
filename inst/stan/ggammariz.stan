...
psi = (to_matrix(exp(-phi * (nodesf * alpha + row_sums(nodesr .* row_expan0(b,qpoints)))), m, qpoints, 0) 
   * weights) .* st .* exp(-pred0) * 0.5;
for(i in 1:m){
llik[i] = status[i] == 1 ? (log(kappa) + kappa * gam * log(rho) - lgamma(gam) 
   + (kappa * gam - 1) * log(psi[i]) - pow((rho * psi[i]),kappa) - pred1[i])
   :  log(gamma_q(gam, pow((rho*psi[i]),kappa)));
}
