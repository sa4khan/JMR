########################################
# Recurrent: Negative log-likelihood of Weibull PH
###########################################
Rwph.llik<-function(init,tstart,tstop,status,xx){
p<-ncol(xx)
beta<-init[1:p]
kappa<-exp(init[p+1])
rho<-exp(init[p+2])
pred<-as.matrix(xx)%*%beta
lf<-status*(log(kappa)+kappa*log(rho)+(kappa-1)*log(tstop)+pred)-
  rho^kappa*(tstop^kappa-tstart^kappa)*exp(pred)
return(-sum(lf))
}
#####################################################
# Recurrent: Fit of the weibull AFT.
#################################################
Rwph.fit<-function(Formula,init=NULL,data,conf.int=0.95,iter.max=250){
...
k<-k+1
options(warn=-1)
fit1<-tryCatch({nlminb(start=init[k,],objective=Rwph.llik,tstart=tstart,tstop=tstop,
...
k<-k+1
options(warn=-1)
fit1<-tryCatch({optim(par=init[k,],fn=Rwph.llik,tstart=tstart,tstop=tstop,
...
rownames(dat.sum)<-""
model<-noquote(rbind("Weibull PH","rho = rate parameter", "kappa = shape parameter"))
colnames(model)<-""
...
#################################################
Rwph.resid<-function(fit,plot=FALSE,conf.int=0.95,xlim=NULL,ylim=NULL,
...
pred<-as.matrix(xx)%*%beta
Rhat1<-(rho*tstart)^kappa*exp(pred)
Rhat2<-(rho*tstop)^kappa*exp(pred)
