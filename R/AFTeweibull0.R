...
####################################################
ewaft.llik<-function(init,st,status,xx){
p<-ncol(xx)
beta<-init[1:p]
kappa<-exp(init[p+1])
gam<-exp(init[p+2])
rho<-exp(init[p+3])
pred<-c(as.matrix(xx)%*%beta)
u<-st*exp(-pred)
s1<-log1mexp(-(rho*u)^kappa)
r1<-log1p(-exp(gam*s1))
if(any(!is.finite(r1))){
wh<-which(!is.finite(r1))
r1[wh]<-log(gam)-(rho*u[wh])^kappa
}
lf<-status*(log(kappa)+log(gam)+kappa*log(rho)+(kappa-1)*log(u)+
    (gam-1)*s1-(rho*u)^kappa-pred-r1)+r1
neg.lf<--sum(lf)
return(neg.lf)
}
#######################################################
...
#################################################
ewaft.fit<-function(Formula,init=NULL,data,conf.int=0.95,iter.max=250){
...
options(warn=-1)
hess<-fdHess(pars=fit1$par,fun=ewaft.llik,st=st,status=status,xx=xx)$Hessian
options(warn=0)
...
if(code==1){
options(warn=-1)
hess<-hessian(func=ewaft.llik,x=fit1$par,st=st,status=status,xx=xx)
options(warn=0)
if(all(is.finite(hess))){
if(!any(eigen(hess)$values<=0)){
logL<--fit1$objective
...
result3<-cbind("exp(-coef)"=exp(-est)[1:p],"lower.95"=exp(-ci.up)[1:p],upper.95=exp(-ci.lo)[1:p])
...
rownames(dat.sum)<-""
model<-noquote(rbind("Exponentiated Weibull AFT","rho = rate parameter", 
...
#################################################
ewaft.resid<-function(fit,plot=FALSE,conf.int=0.95,xlim=NULL,ylim=NULL,
...
u<-st*exp(-pred)
ss1<-log1mexp(-mpfr((rho*u)^kappa,500))
s1<-new("mpfr", unlist(ss1))
Rhat<--as.numeric(log1p(-exp(gam*s1)))
