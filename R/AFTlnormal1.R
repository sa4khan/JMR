...
####################################################
Rlnaft.llik<-function(init,tstart,tstop,status,xx){
p<-ncol(xx)
beta<-init[1:p]
kappa<-exp(init[p+1])
rho<-exp(init[p+2])
...
#################################################
Rlnaft.fit<-function(Formula,init=NULL,data,conf.int=0.95,iter.max=250){
...
options(warn=-1)
hess<-fdHess(pars=fit1$par,fun=Rlnaft.llik,tstart=tstart,tstop=tstop,
...
#################################################
Rlnaft.resid<-function(fit,plot=FALSE,conf.int=0.95,xlim=NULL,ylim=NULL,
