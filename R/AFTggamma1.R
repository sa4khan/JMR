...
####################################################
Rggamma.llik<-function(init,tstart,tstop,status,xx){
p<-ncol(xx)
beta<-init[1:p]
kappa<-exp(init[p+1])
gam<-exp(init[p+2])
...
u0<-tstart*exp(-pred)
u1<-tstop*exp(-pred)
log.st0<-pgamma((rho*u0)^kappa,shape=gam,rate=1,lower.tail=FALSE,log=TRUE)
log.st1<-pgamma((rho*u1)^kappa,shape=gam,rate=1,lower.tail=FALSE,log=TRUE)
log.ft1<-log(kappa)+kappa*gam*log(rho)+(kappa*gam-1)*log(u1)-(rho*u1)^kappa-lgamma(gam)-pred
...
#################################################
Rggamma.fit<-function(Formula,init=NULL,data,conf.int=0.95,iter.max=250){
...
k<-k+1
options(warn=-1)
fit1<-tryCatch({nlminb(start=init[k,],objective=Rggamma.llik,tstart=tstart,tstop=tstop,
...
options(warn=-1)
hess<-fdHess(pars=fit1$par,fun=Rggamma.llik,tstart=tstart,tstop=tstop,
...
if(code==1){
options(warn=-1)
hess<-hessian(func=Rggamma.llik,x=fit1$par,tstart=tstart,tstop=tstop,
  status=status,xx=xx)
options(warn=0)
if(all(is.finite(hess))){
if(!any(eigen(hess)$values<=0)){
...
k<-k+1
options(warn=-1)
fit1<-tryCatch({optim(par=init[k,],fn=Rggamma.llik,tstart=tstart,tstop=tstop,
...
#################################################
Rggaft.resid<-function(fit,plot=FALSE,conf.int=0.95,xlim=NULL,ylim=NULL,
