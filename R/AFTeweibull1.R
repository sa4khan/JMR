...
####################################################
Rewaft.llik<-function(init,tstart,tstop,status,xx){
p<-ncol(xx)
beta<-init[1:p]
kappa<-exp(init[p+1])
gam<-exp(init[p+2])
...
u0<-tstart*exp(-pred)
u1<-tstop*exp(-pred)
s0<-log1mexp(-(rho*u0)^kappa)
s1<-log1mexp(-(rho*u1)^kappa)
r0<-log1p(-exp(gam*s0))
r1<-log1p(-exp(gam*s1))
if(any(!is.finite(r0))){
wh<-which(!is.finite(r0))
r0[wh]<-log(gam)-(rho*u0[wh])^kappa
}
if(any(!is.finite(r1))){
wh<-which(!is.finite(r1))
r1[wh]<-log(gam)-(rho*u1[wh])^kappa
}
lf<-status*(log(kappa)+log(gam)+kappa*log(rho)+(kappa-1)*log(u1)+
  (gam-1)*s1-(rho*u1)^kappa-pred-r1)-(r0-r1)
neg.lf<--sum(lf)
return(neg.lf)
}
#######################################################
...
#################################################
Rewaft.fit<-function(Formula,init=NULL,data,conf.int=0.95,iter.max=250){
...
options(warn=-1)
hess<-fdHess(pars=fit1$par,fun=Rewaft.llik,tstart=tstart,tstop=tstop,
...
#################################################
Rewaft.resid<-function(fit,plot=FALSE,conf.int=0.95,xlim=NULL,ylim=NULL,
...
u0<-tstart*exp(-pred)
u1<-tstop*exp(-pred)
ss0<-log1mexp(-mpfr((rho*u0)^kappa,500))
ss1<-log1mexp(-mpfr((rho*u1)^kappa,500))
s0<-new("mpfr", unlist(ss0))
s1<-new("mpfr", unlist(ss1))
Rhat1<--as.numeric(log1p(-exp(gam*s0)))
Rhat2<--as.numeric(log1p(-exp(gam*s1)))
