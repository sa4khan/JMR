########################################
# Recurrent: Negative log-likelihood of log-logistic AFT
###########################################
Rllaft.llik<-function(init,tstart,tstop,status,xx){
p<-ncol(xx)
beta<-init[1:p]
kappa<-exp(init[p+1])
rho<-exp(init[p+2])
...
u1<-tstop*exp(-pred)
u0<-tstart*exp(-pred)
lf<-status*(log(kappa)+kappa*log(rho)+(kappa-1)*log(u1)-pred-log(1+(rho*u1)^kappa))-
    (log(1+(rho*u1)^kappa)-log(1+(rho*u0)^kappa))
return(-sum(lf))
}
#####################################################
# Recurrent: Fit of the log-logistic AFT.
#################################################
Rllaft.fit<-function(Formula,init=NULL,data,conf.int=0.95,iter.max=250){
...
k<-k+1
options(warn=-1)
fit1<-tryCatch({nlminb(start=init[k,],objective=Rllaft.llik,tstart=tstart,tstop=tstop,
...
options(warn=-1)
hess<-fdHess(pars=fit1$par,fun=Rllaft.llik,tstart=tstart,tstop=tstop,
...
if(code==1){
options(warn=-1)
hess<-hessian(func=Rllaft.llik,x=fit1$par,tstart=tstart,tstop=tstop,
  status=status,xx=xx)
options(warn=0)
if(all(is.finite(hess))){
if(!any(eigen(hess)$values<=0)){
...
k<-k+1
options(warn=-1)
fit1<-tryCatch({optim(par=init[k,],fn=Rllaft.llik,tstart=tstart,tstop=tstop,
...
#################################################
Rllaft.resid<-function(fit,plot=FALSE,conf.int=0.95,xlim=NULL,ylim=NULL,
...
u1<-tstop*exp(-pred)
u0<-tstart*exp(-pred)
Rhat1<-log(1+(rho*u0)^kappa)
Rhat2<-log(1+(rho*u1)^kappa)
