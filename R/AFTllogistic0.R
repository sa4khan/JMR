########################################
# Negative log-likelihood of Log-logistic AFT
###########################################
llaft.llik<-function(init,st,status,xx){
p<-ncol(xx)
beta<-init[1:p]
kappa<-exp(init[p+1])
rho<-exp(init[p+2])
...
u<-st*exp(-pred)
lf<-status*(log(kappa)+kappa*log(rho)+(kappa-1)*log(u)-pred-
  log(1+(rho*u)^kappa))-log(1+(rho*u)^kappa)
return(-sum(lf))
}
#######################################################
# Fit of the log-logistic AFT.
#################################################
llaft.fit<-function(Formula,init=NULL,data,conf.int=0.95,iter.max=250){
...
k<-k+1
options(warn=-1)
fit1<-tryCatch({nlminb(start=init[k,],objective=llaft.llik,st=st,
...
options(warn=-1)
hess<-fdHess(pars=fit1$par,fun=llaft.llik,st=st,status=status,xx=xx)$Hessian
options(warn=0)
...
if(code==1){
options(warn=-1)
hess<-hessian(func=llaft.llik,x=fit1$par,st=st,status=status,xx=xx)
options(warn=0)
if(all(is.finite(hess))){
if(!any(eigen(hess)$values<=0)){
logL<--fit1$objective
...
k<-k+1
options(warn=-1)
fit1<-tryCatch({optim(par=init[k,],fn=llaft.llik,st=st,
  status=status,xx=xx,method="BFGS",hessian=TRUE,control=list(maxit=iter.max))},
  error = function(e){NULL})
options(warn=0)
if(!is.null(fit1)){
hess<-fit1$hessian
if(all(is.finite(hess)) & fit1$convergence==0 & is.finite(fit1$value) & 
    fit1$value!=0 & !is.na(fit1$value)){
if(!any(eigen(hess)$values<=0)){
code<-0
logL<--fit1$value
optimizer<-"optim"}}}
...
rownames(dat.sum)<-""
model<-noquote(rbind("Log-logistic AFT","rho = rate parameter", "kappa = shape parameter"))
colnames(model)<-""
...
results<-list(model=model,
  ...
  st=st,status=status,design.mat=xx,surv.model="llaft",code=0)
}
class(results)<-"JMR"
...
##################################################
# Residual plot for log-logistic
#################################################
llaft.resid<-function(fit,plot=FALSE,conf.int=0.95,xlim=NULL,ylim=NULL,
