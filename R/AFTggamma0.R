###################################################
# Negative log-likelihood of G gamma AFT
####################################################
ggamma.llik<-function(init,st,status,xx){
p<-ncol(xx)
beta<-init[1:p]
kappa<-exp(init[p+1])
gam<-exp(init[p+2])
...
u<-st*exp(-pred)
log.st1<-pgamma((rho*u)^kappa,shape=gam,rate=1,lower.tail=FALSE,log=TRUE)
log.ft1<-log(kappa)+kappa*gam*log(rho)+(kappa*gam-1)*log(u)-(rho*u)^kappa-lgamma(gam)-pred
...
#######################################################
# Fit of the G gamma AFT.
#################################################
ggamma.fit<-function(Formula,init=NULL,data,conf.int=0.95,iter.max=250){
...
k<-k+1
options(warn=-1)
fit1<-tryCatch({nlminb(start=init[k,],objective=ggamma.llik,st=st,
...
options(warn=-1)
hess<-fdHess(pars=fit1$par,fun=ggamma.llik,st=st,status=status,xx=xx)$Hessian
options(warn=0)
...
if(code==1){
options(warn=-1)
hess<-hessian(func=ggamma.llik,x=fit1$par,st=st,status=status,xx=xx)
options(warn=0)
if(all(is.finite(hess))){
if(!any(eigen(hess)$values<=0)){
logL<--fit1$objective
...
k<-k+1
options(warn=-1)
fit1<-tryCatch({optim(par=init[k,],fn=ggamma.llik,st=st,
...
rownames(result4)<-""
dat.sum<-cbind(n=length(st),"events"=sum(status),censored=sum(1-status),predictors=p)
rownames(dat.sum)<-""
model<-noquote(rbind("Generalized Gamma AFT","rho = rate parameter", "kappa, gamma = shape parameters"))
colnames(model)<-""
...
results<-list(model=model,
  ...
  st=st,status=status,design.mat=xx,surv.model=surv.model0,code=0)
}
class(results)<-"JMR"
...
##################################################
# Residual plot for ggamma
#################################################
ggaft.resid<-function(fit,plot=FALSE,conf.int=0.95,xlim=NULL,ylim=NULL,
...
Rhat<--pgamma((rho*u)^kappa,shape=gam,rate=1,lower.tail=FALSE,log=TRUE)
