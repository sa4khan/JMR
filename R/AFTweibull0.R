########################################
# Negative log-likelihood of Weibull AFT
###########################################
waft.llik<-function(init,st,status,xx){
p<-ncol(xx)
beta<-init[1:p]
kappa<-exp(init[p+1])
rho<-exp(init[p+2])
pred<-as.matrix(xx)%*%beta
lf<-status*(log(kappa)+kappa*log(rho)+(kappa-1)*log(st)-(kappa-1)*pred-pred)-
  exp(-kappa*pred)*(st^kappa)*(rho^kappa)
return(-sum(lf))
}
#######################################################
# Fit of the weibull AFT.
#################################################
waft.fit<-function(Formula,init=NULL,data,conf.int=0.95,iter.max=250){
...
k<-k+1
options(warn=-1)
fit1<-tryCatch({nlminb(start=init[k,],objective=waft.llik,st=st,
...
options(warn=-1)
hess<-fdHess(pars=fit1$par,fun=waft.llik,st=st,status=status,xx=xx)$Hessian
options(warn=0)
...
if(code==1){
options(warn=-1)
hess<-hessian(func=waft.llik,x=fit1$par,st=st,status=status,xx=xx)
options(warn=0)
if(all(is.finite(hess))){
if(!any(eigen(hess)$values<=0)){
logL<--fit1$objective
...
k<-k+1
options(warn=-1)
fit1<-tryCatch({optim(par=init[k,],fn=waft.llik,st=st,
...
results<-list(model=model,
  ...
  st=st,status=status,design.mat=xx,surv.model="waft",code=0)
}
class(results)<-"JMR"
...
##################################################
# Residual plot for weibull
#################################################
waft.resid<-function(fit,plot=FALSE,conf.int=0.95,xlim=NULL,ylim=NULL,
