jmfit.3par<-function(surv.fit,lme.fit,surv.model,
...
#-- E Weibull --
if(surv.model=="eweibull" || surv.model=="ggamma"){
fit<-ewaft.fit(Formula,init=NULL,data=surv.dat.frame)
if(fit$code==0){
...
#-- GLL --
if(surv.model=="gllph"){
fit<-gll.fit(Formula,init=NULL,data=surv.dat.frame)
if(fit$code==0){
kappa<-exp(fit[[3]][(p2+1),1])
gam<-exp(fit[[3]][(p2+2),1])
rho<-exp(fit[[3]][(p2+3),1])
...
inits<-rep(list(list(alpha=con[["alpha"]],beta=con[["beta"]],phi=con[["phi"]],
   squ_kappa=con[["kappa"]]^2,inv_gam2=1/con[["gam"]]^2,inv_rho2=1/con[["rho"]]^2,
   b_unscaled=solve(t(chol(getVarCov(lme.fit))))%*%t(con[["b"]]),
   rand_cov=con[["rand_cov"]],inv_sigma2=1/con[["sigma"]]^2)),chains)
} else{
inits<-rep(list(list(alpha=con[["alpha"]],beta=con[["beta"]],phi=con[["phi"]],
   squ_kappa=con[["kappa"]]^2,squ_gam=con[["gam"]]^2,inv_rho2=1/con[["rho"]]^2,
   b_unscaled=solve(t(chol(getVarCov(lme.fit))))%*%t(con[["b"]]),
   rand_cov=con[["rand_cov"]],inv_sigma2=1/con[["sigma"]]^2)),chains)
}
}
par.stan<-c("beta","phi","rho","kappa","gam","alpha","rand_cov","sigma","b")
# ---------------------------------
# Stan Model
...
data.stan<-list(m=m,n=n,p1=p1,p2=p2,pp1=pp1,
  ...
  c0=con[["c0"]],c1=con[["c1"]],d0=con[["d0"]],d1=con[["d1"]])
#stanc("stan_eweibullrizsimple.stan")
#stan_model <- "stan_eweibullrizsimple.stan"
stan_model<-stanmodels$eweibullrizsimple
} else{
data.stan<-list(m=m,n=n,p1=p1,p2=p2,pp1=pp1,
    ...
    a0=con[["a0"]],a1=con[["a1"]],b0=con[["b0"]],b1=con[["b1"]],
    c0=con[["c0"]],c1=con[["c1"]],d0=con[["d0"]],d1=con[["d1"]])
#stanc("stan_eweibullriz.stan")
#stan_model <- "stan_eweibullriz.stan"
stan_model<-stanmodels$eweibullriz
}
...
data.stan<-list(m=m,n=n,p1=p1,p2=p2,pp1=pp1,
  ...
  a0=con[["a0"]],a1=con[["a1"]],b0=con[["b0"]],b1=con[["b1"]],
  c0=con[["c0"]],c1=con[["c1"]],d0=con[["d0"]],d1=con[["d1"]])
#stanc("stan_ggammarizsimple.stan")
#stan_model <- "stan_ggammarizsimple.stan"
stan_model<-stanmodels$ggammarizsimple
} else{
data.stan<-list(m=m,n=n,p1=p1,p2=p2,pp1=pp1,
    qpoints=con[["qpoints"]],id_freq=id_freq,
    y=y,st=st,status=status,
    ...
    a0=con[["a0"]],a1=con[["a1"]],b0=con[["b0"]],b1=con[["b1"]],
    c0=con[["c0"]],c1=con[["c1"]],d0=con[["d0"]],d1=con[["d1"]])
#stanc("stan_ggammariz.stan")
#stan_model <- "stan_ggammariz.stan"
stan_model<-stanmodels$ggammariz
}
method0<-rbind("Induced by the longitudinal value.",
  "Rizopoulos D (2012). Joint Models for Longitudinal and Time-to-Event", 
  "       Data With Applications in R, Chapman and Hall/CRC.",
...
data.stan<-list(m=m,n=n,p1=p1,p2=p2,pp1=pp1,
    ...
    a0=con[["a0"]],a1=con[["a1"]],b0=con[["b0"]],b1=con[["b1"]],
    c0=con[["c0"]],c1=con[["c1"]],d0=con[["d0"]],d1=con[["d1"]])
#stanc("stan_gllphriz.stan")
#stan_model <- "stan_gllphriz.stan"
stan_model<-stanmodels$gllphriz
method0<-rbind("Induced by the longitudinal value.",
  "Rizopoulos D (2012). Joint Models for Longitudinal and Time-to-Event", 
  "       Data With Applications in R, Chapman and Hall/CRC.")
colnames(method0)<-""
...
data.stan<-list(m=m,n=n,p1=p1,p2=p2,pp1=pp1,
  ...
  a0=con[["a0"]],a1=con[["a1"]],b0=con[["b0"]],b1=con[["b1"]],
  c0=con[["c0"]],c1=con[["c1"]],d0=con[["d0"]],d1=con[["d1"]])
#stanc("stan_eweibullsimple.stan")
#stan_model <- "stan_eweibullsimple.stan"
stan_model<-stanmodels$eweibullsimple
} else{
data.stan<-list(m=m,n=n,p1=p1,p2=p2,pp1=pp1,
    qpoints=con[["qpoints"]],id_freq=id_freq,
    y=y,st=st,status=status,
    ...
    a0=con[["a0"]],a1=con[["a1"]],b0=con[["b0"]],b1=con[["b1"]],
    c0=con[["c0"]],c1=con[["c1"]],d0=con[["d0"]],d1=con[["d1"]])
#stanc("stan_eweibull.stan")
#stan_model <- "stan_eweibull.stan"
stan_model<-stanmodels$eweibull
}
method0<-rbind("Induced by the longitudinal value.",
...
data.stan<-list(m=m,n=n,p1=p1,p2=p2,pp1=pp1,
  ...
  a0=con[["a0"]],a1=con[["a1"]],b0=con[["b0"]],b1=con[["b1"]],
  c0=con[["c0"]],c1=con[["c1"]],d0=con[["d0"]],d1=con[["d1"]])
#stanc("stan_ggammasimple.stan")
#stan_model <- "stan_ggammasimple.stan"
stan_model<-stanmodels$ggammasimple
} else{
data.stan<-list(m=m,n=n,p1=p1,p2=p2,pp1=pp1,
    qpoints=con[["qpoints"]],id_freq=id_freq,
    y=y,st=st,status=status,
    ...
    a0=con[["a0"]],a1=con[["a1"]],b0=con[["b0"]],b1=con[["b1"]],
    c0=con[["c0"]],c1=con[["c1"]],d0=con[["d0"]],d1=con[["d1"]])
#stanc("stan_ggamma.stan")
#stan_model <- "stan_ggamma.stan"
stan_model<-stanmodels$ggamma
}
method0<-rbind("Induced by the longitudinal value.",
  "Guo X and Carlin BP (2004). Separate and Joint Modeling of Longitudinal", 
  "       and Event Time Data Using Standard Computer Packages,",
...
data.stan<-list(m=m,n=n,p1=p1,p2=p2,pp1=pp1,
    ...
    a0=con[["a0"]],a1=con[["a1"]],b0=con[["b0"]],b1=con[["b1"]],
    c0=con[["c0"]],c1=con[["c1"]],d0=con[["d0"]],d1=con[["d1"]])
#stanc("stan_gllph.stan")
#stan_model <- "stan_gllph.stan"
stan_model<-stanmodels$gllph
method0<-rbind("Induced by the longitudinal value.",
  "Guo X and Carlin BP (2004). Separate and Joint Modeling of Longitudinal", 
  "       and Event Time Data Using Standard Computer Packages,",
  "       The American Statistician, 58: 1 - 9.",
...
sum.stanfit<-rstan::summary(stan_fit,pars=c("beta","phi","rho","kappa","gam","alpha","rand_cov","sigma"))$summary
...
kappa.loc<-p2+3
gam.loc<-p2+4
alpha.loc<-(p2+5):(p2+4+p1)
cov.loc<-(p2+5+p1):(p2+4+p1+pp1^2)
sigma.loc<-p2+5+p1+pp1^2
b.loc<-(p2+6+p1+pp1^2):(p2+5+p1+pp1^2+pp1*m)
#---------------------------------------------------
# Save data
...
stan.data<-list(m=m,n=n,p1=p1,p2=p2,pp1=pp1,id=idd,
    ...
    gam=con[["gam"]],d0=con[["d0"]],d1=con[["d1"]])
} else{
stan.data<-list(m=m,n=n,p1=p1,p2=p2,pp1=pp1,id=idd,
    qpoints=con[["qpoints"]],id_freq=id_freq,
    y=y,st=st,status=status,
...
post.mean<-list(beta=sum.stanfit[beta.loc,1],phi=sum.stanfit[phi.loc,1],
  kappa=sum.stanfit[kappa.loc,1],rho=sum.stanfit[rho.loc,1],gam=sum.stanfit[gam.loc,1],
...
post.med<-list(beta=sum.stanfit[beta.loc,6],phi=sum.stanfit[phi.loc,6],
  kappa=sum.stanfit[kappa.loc,6],rho=sum.stanfit[rho.loc,6],gam=sum.stanfit[gam.loc,6],
...
if(surv.model=="eweibull"){
results[["surv.model"]]<-"eweibull"
s.model<-rbind("Exponentiated Weibull AFT", "rho = rate parameter","kappa, gamma = shape parameters",
...
if(surv.model=="ggamma"){
results[["surv.model"]]<-"ggamma"
s.model<-rbind("Generalized gamma AFT", "rho = rate parameter","kappa, gamma = shape parameters",
  "beta = regression coefficient vector",
  "phi = association parameter")
colnames(s.model)<-""
rownames(s.model)<-c("","","","","")
...
if(surv.model=="gllph"){
results[["surv.model"]]<-"gllph"
s.model<-rbind("Generalized log-logistic PH", 
  "rho, kappa and gamma are parameters associated with the distribution",
  "Includes as special cases the log-logistic (rho = gamma)",
  "    and Weibull (gamma = 0) distributions",
  "beta = regression coefficient vector",
  "phi = association parameter")
colnames(s.model)<-""
rownames(s.model)<-c("","","","","","")
