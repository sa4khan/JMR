...
#################################################
lnaft.fit<-function(Formula,init=NULL,data,conf.int=0.95,iter.max=250){
...
options(warn=-1)
hess<-fdHess(pars=fit1$par,fun=lnaft.llik,st=st,status=status,xx=xx)$Hessian
options(warn=0)
...
#################################################
lnaft.resid<-function(fit,plot=FALSE,conf.int=0.95,xlim=NULL,ylim=NULL,
...
km.Rhat<-survfit(Surv(Rhat,status)~1,type="kaplan-meier",conf.int=conf.int)
if(plot){
