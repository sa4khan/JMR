...
####################################################
gll.llik<-function(init,st,status,xx){
p<-ncol(xx)
beta<-init[1:p]
kappa<-exp(init[p+1])
gam<-exp(init[p+2])
...
lf<-status*(log(kappa)+kappa*log(rho)+(kappa-1)*log(st)-log(1+(gam*st)^kappa)+pred)-
   (rho/gam)^kappa*log(1+(gam*st)^kappa)*exp(pred)
return(-sum(lf))
}
#######################################################
# Fit of the GLL PH.
#################################################
gll.fit<-function(Formula,init=NULL,data,conf.int=0.95,iter.max=250){
...
k<-k+1
options(warn=-1)
fit1<-tryCatch({nlminb(start=init[k,],objective=gll.llik,st=st,
...
options(warn=-1)
hess<-fdHess(pars=fit1$par,fun=gll.llik,st=st,status=status,xx=xx)$Hessian
options(warn=0)
...
if(code==1){
options(warn=-1)
hess<-hessian(func=gll.llik,x=fit1$par,st=st,status=status,xx=xx)
options(warn=0)
if(all(is.finite(hess))){
if(!any(eigen(hess)$values<=0)){
logL<--fit1$objective
...
k<-k+1
options(warn=-1)
fit1<-tryCatch({optim(par=init[k,],fn=gll.llik,st=st,
...
results<-list(model=model,
  ...
  st=st,status=status,design.mat=xx,surv.model="gllph",code=0)
}
class(results)<-"JMR"
...
#################################################
gll.resid<-function(fit,plot=FALSE,conf.int=0.95,xlim=NULL,ylim=NULL,
...
pred<-as.matrix(xx)%*%beta
Rhat<-(rho/gam)^kappa*log(1+(gam*st)^kappa)*exp(pred)
