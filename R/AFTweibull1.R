########################################
# Recurrent: Negative log-likelihood of Weibull AFT
###########################################
Rwaft.llik<-function(init,tstart,tstop,status,xx){
p<-ncol(xx)
beta<-init[1:p]
kappa<-exp(init[p+1])
rho<-exp(init[p+2])
pred<-as.matrix(xx)%*%beta
lf<-status*(log(kappa)+kappa*log(rho)+(kappa-1)*log(tstop)-(kappa-1)*pred-pred)-
  exp(-kappa*pred)*(tstop^kappa-tstart^kappa)*rho^kappa
return(-sum(lf))
}
#####################################################
# Recurrent: Fit of the weibull AFT.
#################################################
Rwaft.fit<-function(Formula,init=NULL,data,conf.int=0.95,iter.max=250){
...
k<-k+1
options(warn=-1)
fit1<-tryCatch({nlminb(start=init[k,],objective=Rwaft.llik,tstart=tstart,tstop=tstop,
...
options(warn=-1)
hess<-fdHess(pars=fit1$par,fun=Rwaft.llik,tstart=tstart,tstop=tstop,
...
if(code==1){
options(warn=-1)
hess<-hessian(func=Rwaft.llik,x=fit1$par,tstart=tstart,tstop=tstop,
  status=status,xx=xx)
options(warn=0)
if(all(is.finite(hess))){
if(!any(eigen(hess)$values<=0)){
...
k<-k+1
options(warn=-1)
fit1<-tryCatch({optim(par=init[k,],fn=Rwaft.llik,tstart=tstart,tstop=tstop,
...
result1<-data.frame(est=est,se=se,z=z,
  "p value"=ifelse(pval<0.001,"<0.001",round(pval,digits=3)),lower=ci.lo,upper=ci.up)
...
result2<-data.frame(exp.est=exp(est)[1:p],"lower"=exp(ci.lo)[1:p],upper=exp(ci.up)[1:p])
...
result3<-cbind("exp.est"=exp(-est)[1:p],"lower"=exp(-ci.up)[1:p],upper=exp(-ci.lo)[1:p])
...
rownames(dat.sum)<-""
model<-noquote(rbind("Weibull AFT","rho = rate parameter", "kappa = shape parameter"))
colnames(model)<-""
...
##################################################
# Recurrent: Residual plot for weibull
#################################################
Rwaft.resid<-function(fit,plot=FALSE,conf.int=0.95,xlim=NULL,ylim=NULL,
...
pred<-as.matrix(xx)%*%beta
Rhat1<-(rho*tstart*exp(-pred))^kappa
Rhat2<-(rho*tstop*exp(-pred))^kappa
